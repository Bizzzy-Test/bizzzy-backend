...
//     // Regular filters based on user role (Job or client)
//     if (user.role === 1) {
//         if (job_type) {
...
// const searchJob = async (req, userData, res) => {
//   return new Promise(async () => {
//     let { searchTerm, category, experience, contractType } = req?.query;
//     let query = {};

//     if (category) {
//       category = category.split(",");
//       const categoriesPromises = category.map(async (id) => {
//         return await JobSchema.find({ "categories._id": id });
//       });
//     }
...
//         { title: { $regex: new RegExp(searchTerm, "i"), $options: "i" } },
//         { description: { $regex: new RegExp(searchTerm, "i"), $options: "i" } },
//       ];
//     }
...
//             query['sub_categories._id'] = { $in: category };
//         }
//         result = await JobSchema.find(query);
//         logger.info('Job search successfully');
//         return responseData.success(res, result, 'Job search successfully');
//     } catch (err) {
//         logger.error(`${messageConstants.INTERNAL_SERVER_ERROR}. ${err}`);
...
const searchJobPost = async (req, userData, res) => {
    return new Promise(async () => {
        try {
            ...
            if (contractType) {
                query.job_type = new RegExp(contractType, "i");
            }
